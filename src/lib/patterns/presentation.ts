...
export function getPatternDisplayName(pattern: PatternIdentity): string {
    ...
    if (pattern.collectionId === 'minecraft') return `Minecraft ${pattern.title}`;
    if (pattern.collectionId === 'super-mario') return pattern.title === 'Mario' ? 'Super Mario' : `Super Mario ${pattern.title}`;
    if (pattern.collectionId === 'kirby') return pattern.title === 'Kirby' || pattern.title.startsWith('Kirby ') ? pattern.title : `Kirby ${pattern.title}`;
