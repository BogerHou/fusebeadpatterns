        ...
        <>
            ...
            <article>
                ...
                <div className="mt-6 grid items-start gap-6 lg:grid-cols-2 lg:gap-10">
                    ...
                    <div>
                        <p className="text-base leading-7 text-gray-800 sm:text-lg">{pattern.description}</p>
                        ...
                        <div className="flex flex-wrap gap-3">
                            <a href={pattern.assets.pdf} download className="inline-flex min-h-11 items-center border-2 border-brutal-black bg-brand-yellow px-4 py-2 font-bold shadow-brutal-sm hover:bg-white">Download PDF</a>
                            <a href={pattern.assets.grid} download className="inline-flex min-h-11 items-center border-2 border-brutal-black bg-white px-4 py-2 font-bold hover:bg-brand-cyan">Download grid PNG</a>
                        </div>
                        ...
                        <Link href={`/editor?pattern=${pattern.id}`} prefetch={false} className="mt-5 inline-flex min-h-11 items-center font-bold underline decoration-2 underline-offset-4">Open in editor <span aria-hidden="true" className="ml-2">→</span></Link>
                        <p className="text-sm leading-6 text-gray-600">Adjust individual beads or colors, then save your own version.</p>
                    </div>
                </div>
                ...
                <details className="mt-6 border-t border-gray-400 pt-4 text-sm leading-6">
                    ...
                    <div className="mt-3 flex flex-wrap gap-x-6 gap-y-2"><a href={pattern.assets.project} download className="underline underline-offset-4">Download editable project</a><a href={pattern.assets.pixels} download className="underline underline-offset-4">Download pattern pixels</a></div>
                </details>
            </article>
        ...
        <>
            ...
            <main className="mx-auto w-full max-w-6xl flex-1 px-3 pb-10 sm:px-4 sm:pb-12">
                {pattern ? <PatternDetail pattern={pattern} /> : <>
                    ...
                    <p className="mb-6 mt-4 max-w-3xl text-base leading-7 text-gray-700 sm:text-lg">{collection!.description}</p>
                    <PatternGrid patterns={getPatternsForCollection(collection!.id).map(toPatternCard)} />
